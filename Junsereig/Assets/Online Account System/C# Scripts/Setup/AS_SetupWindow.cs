...
{ 
	...
	void CredentialsGUI () {
		...
		if (GUI.Button (new Rect (420, 330, 120, 30), "Next")){
            ...
            if (!_phpScriptsLocation.Contains(cmp))
            {
                _phpScriptsLocation = cmp + _phpScriptsLocation;
                this.Log(LogType.Warning, "Automatically added 'http://' in front of the php script location..!");
            }

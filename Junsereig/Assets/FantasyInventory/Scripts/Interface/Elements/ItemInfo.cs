    ...
    public class ItemInfo : MonoBehaviour
    {
        ...
        public void Initialize(ItemId itemId, ItemParams itemParams, bool shop = false)
        {
            ...
            foreach (var attribute in itemParams.Properties)
            {
                ...
                {
                    speedStage = (1 / (basicAttack.autoAttackCooldown - (float)(attribute.Value / 100)));

                    if (basicAttack.AttackSpeed < speedStage)
                    {
                        mod = "-";
                    }
                    else
                    ...
                    description.Add(string.Format("{0}: {1}", SplitName(attribute.Id.ToString()), mod + speedStage.ToString("F2")));
                }
                else
                {
